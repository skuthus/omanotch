...
test("state resolution ranks calibrate > event > expanded > compact > idle", () => {
  ...
  assert.equal(M.resolveState({ activity: true, expanded: true, mediaPlaying: true }), "activity")
  assert.equal(M.resolveState({ activity: true, event: { kind: "osd" }, expanded: true }), "event")
  assert.equal(M.resolveState({ activity: true, event: { kind: "flash" } }), "event")
  assert.equal(M.dictationLabel("recording"), "Listening")
  assert.equal(M.dictationLabel("transcribing"), "Transcribing")
  assert.equal(M.dictationLabel("idle"), "")
})
...
test("island sizes grow symmetrically from the notch", () => {
  ...
  assert.equal(M.islandSize("activity", s, { textWidth: 60, pad: 12 }).height, 32)
  assert.ok(M.islandSize("activity", s, { textWidth: 60, pad: 12 }).width > 200)
